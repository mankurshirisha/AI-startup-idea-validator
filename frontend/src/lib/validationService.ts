...
export interface BackendValidationResponse {
  ...
  comparison?: {
    ...
    /** Personalized mentor-tone recommendations generated by the backend Gemini agent. */
...
export function mapBackendToValidationResult(
  ...
  // 13. Categorized Recommendations — read from backend personalized_recommendations first.
  const pr = comparison.personalized_recommendations ?? {}

  // Helper: pick backend text if non-empty, else use a short contextual fallback.
  const pickRec = (backendText: string | undefined, fallback: string): string =>
    typeof backendText === 'string' && backendText.trim().length > 10
      ? backendText.trim()
      : fallback

  const cust = marketOpp.customerInsights?.targetSegments?.[0] || 'your target customers'
  const ind = industry
  const loc = webSearch.market_size ? `the ${industry} market` : 'your target market'
  ...
  const categorizedRecommendations: CategorizedRecommendation[] = [
    {
      ...
      text: pickRec(
        pr.innovation,
        `Differentiate ${idea} by adding unique features that directly solve the biggest pain points of ${cust} in ${ind}.`,
      ),
    },
    {
      ...
      text: pickRec(
        pr.market_demand,
        `Run a focused pilot with a small group of ${cust} in ${loc} to validate demand before scaling your go-to-market efforts.`,
      ),
    },
    {
      ...
      text: pickRec(
        pr.competition,
        `Identify the single feature gap most competitors in ${ind} leave unaddressed and build your positioning exclusively around it.`,
      ),
    },
    {
      ...
      text: pickRec(
        pr.scalability,
        `Design your core product workflows to serve additional customer segments beyond ${cust} as the business expands into new markets.`,
      ),
    },
    {
      ...
      impact: 'High Impact',
      text: pickRec(
        pr.technical_feasibility,
        `Prioritize product reliability, data security, and a simple onboarding experience for ${cust} from the very first release.`,
      ),
    },
    {
      ...
      impact: 'Transformational',
      text: pickRec(
        pr.business_viability,
        `Test at least two pricing tiers with ${cust} early to identify the model that balances acquisition cost and sustainable revenue growth.`,
      ),
    },
  ]
